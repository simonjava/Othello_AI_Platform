...
{
	ChangeBoard();
	fanzhuan(x,y);                                    //棋盘黑白互换，来使函数legalDirect可以直接调用从而调用legal；
	int w=0;
	for(int i(0); i<8; i++)
		for(int j(0); j<8; j++)
			if(legal(i,j))w++;                           
...
void Power(int k)                                       //初始权值计算； 
{
	...
	{
		av[i].power=0;
		if(isConner(av[i].x,av[i].y))av[i].power+=15;
		if(WhiteCanGotoConner(av[i].x,av[i].y))av[i].power-=15;
		if(is1BesideConner(av[i].x,av[i].y))av[i].power-=5;
		if(is2BesideConner(av[i].x,av[i].y))av[i].power+=5;
	}
}
 ...
 int main()
{
	...
	bool key1=0, key2=0;                                 //第一阶段，第二阶段，第三阶段
	if(board[0][0]!=0||board[0][7]!=0||board[7][0]!=0||board[7][7]!=0)key2=1;  //第三阶段的判别； 
	                                                            
	if(w>16)key1=1;											//第二阶段的判断； 
	int k;
	k=GiveAv(av);
	Power(k);
	
	
	if(key2==1)										//第二阶段，吃掉尽可能多的棋子； 
	{    
		for(int i(0); i<k; i++)
		{     
			av[i].power+=CanEatNum(av[i].x,av[i].y)*2;
