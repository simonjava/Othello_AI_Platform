...
int istype1 (int i, int j) {return ((i==1&&j==1) || (i==8&&j==8) || (i==1&&j==8) ||(i==8&&j==1)); }
...
int point (int i,int j, int T)
{
    ...
    if (f==0)//对手不能落子了
        return 10000;
    if (T<BEGIN) {
        return 2*(g1-0.4*g2max) + 3*(c1-0.3*c2max) -(flag==1?10:0) - (boom==1?150:0);
    }
    else if (T >= BEGIN && T<MID1) {
        return  3*(g1-0.4*g2max) + (c1-0.3*c2max)-(flag==1?10:0) - (boom==1?150:0) - bigboom*200;
    }
    else if (T>=MID1 && T<MID2) {
        return  (g1-0.3*g2max) + 3*(c1-0.3*c2max)-(flag==1?10:0) - (boom==1?150:0) - bigboom*200;
    }
    else
        return (g1-0.2*g2max)+4*(c1-0.45*c2max)-(flag==1?10:0) - (boom==1?150:0)- bigboom*150;
}
