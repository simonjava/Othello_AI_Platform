...
bool WhiteMustGotoConner1(int x ,int y)
{
	CopyBoard();
	int a=x,b=y;
	fanzhuan(a,b);
	ChangeBoard();
	if(WhiteCanGoNum(a,b)!=1)return 0;
	for(int i(0); i<8; i++)
		for(int j(0); j<8; j++)
		{
			int a1=i,b1=j;
			if(is1BesideConner(a1,b1))
				if(legal(a1,b1)){
					CopyBoard();
					return 1; 
				}
...
void Power(int k)                                       //初始权值计算； 
{
	...
	for(int i(0); i<k; i++)
	{
		...
		int x=av[i].x;
		int y=av[i].y;
		if(isConner(x,y))av[i].power+=100;
		if(WhiteCanGotoConner(x,y))av[i].power-=100;
		if(WhiteCanGotoConner2(x,y))av[i].power-=15;
		if(is1BesideConner(x,y))av[i].power-=50;
		if(is2BesideConner(x,y))av[i].power+=15;
		if(CanEatWhiteInConner2(x,y))av[i].power+=20;
		if(WhiteMustGotoConner1(x,y))av[i].power+=100;
	}
}
 ...
 int main()
{
	...
	if(board[0][0]+board[0][7]+board[7][0]+board[7][7]>1)key3=1;		        //第四阶段的判别；                                                            
	if(w>60)key1=1;																//第二阶段的判断； 
	int k;
	k=GiveAv(av);
	...
	{    
		...
		{     
			...
			av[i].power-=WhiteCanGoNum(av[i].x,av[i].y);
		}
		
	}	
	else if(key2==1)															//第三阶段，吃掉尽可能多的棋子； 
	{    
		for(int i(0); i<k; i++)
		{     
			av[i].power+=CanEatNum(av[i].x,av[i].y)*4;
			av[i].power-=WhiteCanGoNum(av[i].x,av[i].y)*2;
		}
		
	}

	else if(key1==1)                                                    	    //尽可能让白子少走； 
	{
		for(int i(0); i<k; i++)
		{
			av[i].power+=CanEatNum(av[i].x,av[i].y);
			av[i].power-=WhiteCanGoNum(av[i].x,av[i].y)*3;
		}
